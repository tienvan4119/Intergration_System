...
namespace TM.API.Services.Cards
{
    public class CardService : BaseService, ICardService
    {
        ...
        {
            var todo = await UnitOfWork.Repository<Todo>().GetAsync(_ => _.Id == request.Id);
            if (todo is null)
        ...
        {
            var card = await UnitOfWork.Repository<Card>().GetAsync(_ => _.Id == request.CardId);
            if (card == null)
                throw new KeyNotFoundException();
            ...
            switch (propertyName)
            {
                ...
                case "duedate":
                    ...
                    card.DueDate = string.IsNullOrEmpty(request.Value)
                                        ? null : card.DueDate;
                    break;
                case "description":
        ...
        public async Task<bool> OrderCard(int cardIdCurrent, int phaseIdMove, AddCardHistoryRequest history)
        {
            ...
            var movePhase = await UnitOfWork.Repository<Phase>().GetAsync(_ => _.Id == phaseIdMove);
        ...
        public async Task<BasicUserResponse> AssignCard(AddCardAssignRequest request, AddCardHistoryRequest history)
        {
            ...
            var user = await UnitOfWork.Repository<User>().GetAsync(_ => _.Id == request.UserId);
            var card = await UnitOfWork.Repository<Card>().GetAsync(_ => _.Id == request.CardId);
        ...
        public async Task<GetTagResponse> AddTag(CardTagRequest request, AddCardHistoryRequest history)
        {
            ...
            var card = await UnitOfWork.Repository<Card>().GetAsync(_ => _.Id == request.CardId);
            var tag = await UnitOfWork.Repository<Tag>().GetAsync(_ => _.Id == request.TagId);
